...
class _HodNotificationsScreenState extends State<HodNotificationsScreen>
  ...
  void initState() {
    ...
    hodId = FirebaseAuth.instance.currentUser?.uid;
  }
  ...
  Future<void> publishNotification() async {
    ...
    if (hodId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Error: HOD not logged in."),
          backgroundColor: Colors.red,
        ),
      );
      return;
  ...
  void openDetails(Map<String, dynamic> data, String docId) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => HodNotificationDetailsScreen(
          data: data,
          docId: docId,
        ),
      ),
  ...
  Widget notificationList(String type, IconData icon) {
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: FirebaseFirestore.instance
          ...
          .where("senderId", isEqualTo: hodId)
          .snapshots(),
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              labelText: "Title",
              hintText: "E.g., Department meeting update",
              border: OutlineInputBorder(
                borderRadius: BorderRadius.circular(12),
              ),
              filled: true,
...
class HodNotificationDetailsScreen extends StatefulWidget {
  final Map<String, dynamic> data;
  final String docId;

  const HodNotificationDetailsScreen({
    super.key,
    required this.data,
    required this.docId,
  });
  ...
  @override
  State<HodNotificationDetailsScreen> createState() =>
      _HodNotificationDetailsScreenState();
}

class _HodNotificationDetailsScreenState
    extends State<HodNotificationDetailsScreen> {
  ...
  late final String? currentHodId;

  @override
  void initState() {
    super.initState();
    currentHodId = FirebaseAuth.instance.currentUser?.uid;
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    "Designation: ${widget.data["senderRole"] ?? "HOD"}",
                    style: const TextStyle(
                      color: Colors.grey,
                      fontSize: 13,
                    ),
  ...
  bool _canManageNotification() {
    return currentHodId != null &&
        widget.data["senderId"]?.toString() == currentHodId;
  }
}
